...
df["Year"] = df["Date"].dt.year #년 데이터 필터링
df["Month"] = df["Date"].dt.month #월 데이터 필터링

new_df = df.groupby(["Year", 'Month'])["Close"].mean().reset_index() # 년과 월을 통해 종가에 대한 평균값을 작성

new_df["Year_Month"] = new_df["Year"].astype(str) + "-" + new_df["Month"].astype(str) # 년-월 단위로 합체

x = new_df["Year_Month"] # x축은 년-월
y = new_df["Close"] # y축은 종가
